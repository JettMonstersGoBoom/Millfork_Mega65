...
void   *_dmt_strdup(const char *s, const char*file, unsigned line)
{
unsigned len = (unsigned)strlen(s)+1;
unsigned char *bff=_dmt_alloc(len,0,file,line);
	memcpy(bff,s,len);
	return bff;
}
...
//	fprintf(stderr,"free %x %s,line %u %u\n",ptr,file,line,node->size);
...
void dmt_dump(FILE *fp) {
  ...
  fprintf(fp, "Total unfreed: %lu bytes\n", (unsigned long)total);
	fprintf(fp, "Total allocated: %f kb\n",(float)total_allocated/1024.0f);
}
